...
@Injectable()
export class TogetherScriptProvider implements IScriptGenerator {
  private readonly logger = new Logger(TogetherScriptProvider.name);
  ...
  async generateScript(request: GenerateScriptRequestDto): Promise<IVideoScript> {
    ...
    try {
      ...
      throw new ScriptGenerationException('together-ai', error as Error);
    }
  }
  ...
  private getClient(): OpenAI {
    if (!this.client) {
      ...
      // TogetherAI exposes an OpenAI-compatible endpoint
